...
class PlacementDb:
    ...
    def create_application(self, student_id: int, drive_id: int) -> tuple[int, bool]:
        ...
        A duplicate application is a success, not an error. Use ON CONFLICT ... DO NOTHING,
        then return the existing row's id with created = False."""
        with self.transaction() as c:
            cur = c.execute(
    ...
    def claim_slot(self, slot_id: int, student_id: int, expected_version: int) -> bool:
        ...
        expected_version, and bump the version when you book."""
    ...
    def record_notification(self, roll_no: str, message: str, dedupe_key: str) -> tuple[int, bool]:
        ...
        with self.transaction() as c:
            ...
            if cur.rowcount == 1:
                return cur.lastrowid, True
            existing = c.execute("SELECT id FROM notification WHERE dedupe_key = ?", (dedupe_key,)).fetchone()
            return existing["id"], False
